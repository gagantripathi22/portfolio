...
const ProjectsArea = () => {
  ...
  const [currentProjectAnimation, setCurrentProjectAnimation] = useState(
    "projectAreaBorder projectAreaBorderMain"
  );
  ...
  const nextProject = () => {
    if (showDownArrow && !isAnimating) {
      ...
      setTimeout(() => {
        setCurrentProjectAnimation(
          "projectAreaBorder projectAreaBorderNoAnimation"
        );
  ...
  useEffect(() => {
    ...
    else setCurrentProjectAnimation("projectAreaBorder projectAreaBorderMain");
  }, [projectAreaExpanded]);

  const techList = (techList) => {
    const techArr = techList.split(", ");
    return techArr.map((item) => (
      <div className="projectExpandDetailDetail">- {item}</div>
    ));
  };

  const handleProjectAreaExpansion = () => {
    if (window.innerWidth > 992 && !isAnimating && !projectAreaExpanded) {
  ...
  return (
    <div className="projectAreaContainer" ref={headingRef}>
      ...
      <div
        className="heading"
        style={{
          left: window.innerWidth < 992 ? prox - 75 : prox - 70,
          top: proy + 520,
        }}
      >
        Projects
      ...
      <div style={{ height: 35 }}>
        ...
        >
          <svg
            className="navBtnIcon navBtnIconRotate"
            height="28"
            fill="white"
            viewBox="0 0 50 50"
            width="50"
      ...
      <div
        className={currentProjectAnimation}
      ...
      >
        <div className="projectArea projectAreaMain" ref={scrollRef}>
          ...
          >
            {projectAreaExpanded && (
              <div
                className="expandExitBtn"
                onClick={() => {
                  setProjectAreaExpanded((prev) => !prev);
                  setShowBigCursor(false);
                }}
            ...
            <div className="projectImageArea">
              <img
                ...
                src={projects[currentProject].data.img}
              />
            </div>
            {projectAreaExpanded && (
              <div
              ...
              >
                ...
                <div className="scrollIndicator">
                  ...
                  <div className="scrollIndicatorFixedPosition">
                    <div className="scrollIndicatorAnimator"></div>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="projectDetailsArea">
            ...
            <span className="projectTechUsedList">
              {projects[currentProject].data.tech}
            </span>
          </div>
          {projectAreaExpanded && (
            <>
              <div className="projectExpandDetail">
                ...
                <div className="projectExpandDetailDetail">
                  {projects[currentProject].data.discription}
                </div>
                {projects[currentProject].data.github ||
                projects[currentProject].data.download ||
                projects[currentProject].data.website ? (
                  <h1
                    className="projectExpandDetailTitle"
                    style={{ marginTop: 50 }}
                  >
                    Links
                  </h1>
                ...
                <div className="linksBtnContainer">
                  ...
                  {projects[currentProject].data.download ? (
                    <a
                      href={projects[currentProject].data.download}
                      style={{ marginRight: 20 }}
                      target="_blank"
                      className="projectDetailGithubBtn"
                    >
                  ...
                  {projects[currentProject].data.website ? (
                    <a
                      href={projects[currentProject].data.website}
                      style={{ marginRight: 20 }}
                      target="_blank"
                      className="projectDetailGithubBtn"
                    >
